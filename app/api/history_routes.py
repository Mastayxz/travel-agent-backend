...
async def get_user_history(
    ...
    try:
        history = list(
            history_collection.find({"google_id": google_id})
        ...
        return {"history": history, "total": history_collection.count_documents({"google_id": google_id})}
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed to retrieve history: {str(e)}")

@router.get("/history/{google_id}/{session_id}")
async def get_chat_session(google_id: str, session_id: str):
    try:
        chat = history_collection.find_one({"google_id": google_id, "session_id": session_id})
        if not chat:
...
@router.put("/history/{google_id}/{session_id}")
async def update_chat_session(google_id: str, session_id: str, history: ChatHistory):
    try:
        data = history.dict()
...
@router.delete("/history/{google_id}/{session_id}")
async def delete_chat_session(google_id: str, session_id: str):
    try:
        result = history_collection.delete_one({"google_id": google_id, "session_id": session_id})
...
@router.post("/history/{google_id}/{session_id}/messages")
